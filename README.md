...
you through them on your own real work, grades the results against
explicit rubrics, and tracks your journey.
...
- **Free tier (shipped here):** Series 1 — *Foundations*, 10 challenges
  ...
  agent" to confident, safe, effective delegation in Cowork.
- **Full curriculum:** 100 challenges across 10 series, ending with
...
- **`CLAUDE.md`** — the companion's operating contract: coach, never
  doer; adapt to the learner's profession; grade honestly; track
  progress truthfully.
- **Skills** (invoked as `/learn-to-claude:<name>`, or triggered
  ...
  - `challenge` — deliver a challenge: lesson taught conversationally,
    task set with parameters and visible rubric
  - `review` — inspect the learner's actual artifacts against the
    rubric; pass/refine with specific feedback; record progress
  - `progress` — the journey map, highlights, and the full-curriculum
    roadmap
...
Each challenge file (`challenges/series-01/`) carries five sections:
**Lesson** (the idea), **Task** (what the learner does), **Parameters**
(constraints that keep it honest), **Rubric** (what passing looks like),
and **Companion notes** (how to coach it). The companion teaches from
these — it never pastes them.
...
2. Where do learners stall (challenge number, refine loops, drop-off)?
3. Does rubric grading feel fair and motivating?
4. Does the roadmap teaser generate pack interest (ask at exit)?

Ask trial users to share their `.learn/progress.json` and impressions
after a week. Known v0.1 caveats are listed in CURRICULUM.md §8 —
notably: verify the plugin CLAUDE.md auto-loads on the tester's Cowork
build; the skills are self-sufficient regardless.
